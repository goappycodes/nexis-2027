    ...
    <main id="top" className="relative pb-20 lg:pb-0">
      ...
      <section className="relative h-[42vh] min-h-[290px] w-full overflow-hidden sm:h-[52vh] lg:h-[60vh]">
        ...
        <div className="shell relative flex h-full flex-col justify-end pb-6 sm:pb-9">
          <Reveal>
            <p className="flex items-center gap-2.5 text-[0.64rem] font-medium uppercase tracking-[0.16em] text-white/80 sm:text-[0.72rem]">
              <span className="h-px w-7 bg-crimson" />
              Undergraduate Program · {BATCH.city}
            </p>
          </Reveal>
          <Reveal delay={80}>
            <h1 className="display balance mt-2.5 max-w-3xl text-[clamp(1.95rem,7.6vw,4.6rem)] text-white">
              Learn business by{" "}
      ...
      <section id="enquire" className="scroll-mt-14 border-b border-line bg-paper-2/40">
        <div className="shell grid gap-6 py-7 sm:gap-8 sm:py-12 lg:grid-cols-12 lg:gap-14">
          {/* Form */}
            ...
            <Reveal>
              <div className="overflow-hidden rounded-[3px] border border-line bg-paper shadow-[0_40px_90px_-55px_rgba(11,12,16,0.55)]">
                <div className="border-b border-line bg-paper-2/60 px-5 py-3.5 sm:px-7 sm:py-4">
                  ...
                  <h2 className="mt-1.5 font-serif text-lg leading-tight sm:text-2xl">
          ...
          <div className="order-2 lg:order-1 lg:col-span-6 lg:pt-2">
            <Reveal>
              <ul className="flex flex-wrap gap-2">
                {PROOF.map((p) => (
                  <li
                    key={p}
                    className="rounded-full border border-line bg-paper px-3 py-1 text-[0.74rem] font-medium text-ink-2 sm:px-3.5 sm:py-1.5 sm:text-[0.82rem]"
                  >
                    {p}
                  </li>
                ))}
            ...
            <Reveal delay={80}>
              <p className="mt-5 max-w-md text-[0.92rem] leading-relaxed text-ink-2 sm:text-[1rem]">
            ...
            <Reveal delay={140}>
              <div className="mt-5 flex items-center gap-3">
                <a
                  ...
                  className="btn flex-1 justify-center border-crimson bg-transparent text-crimson hover:bg-crimson/5"
                >
                  Download prospectus
            ...
            <Reveal delay={200}>
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-3.5">
                  {[
                    ...
                    <img
                      ...
                      src={l.src}
                      alt={l.name}
                      className="h-7 w-auto max-w-[110px] object-contain opacity-80 [filter:brightness(0)] sm:h-9"
                    />
                  ))}
                </div>
              </div>
      ...
      <section className="py-7 sm:py-11">
        <div className="shell mb-3 flex items-end justify-between gap-4">
          <div>
            <p className="kicker flex items-center gap-2.5">
              <span className="h-px w-7 bg-crimson" />
              Inside the campus
            </p>
            <h2 className="display mt-2 text-[clamp(1.4rem,4.8vw,2.6rem)]">
              A campus alive with{" "}
              <span className="serif-em text-crimson">ambition</span>.
            </h2>
          </div>
        </div>
        <div className="flex snap-x snap-mandatory gap-2.5 overflow-x-auto px-[var(--gutter)] pb-2 no-scrollbar">
          {CAMPUS_IMAGES.map((img, i) => (
            <div
              key={img.src}
              className="group relative aspect-[4/3] w-[72%] shrink-0 snap-start overflow-hidden rounded-[2px] bg-paper-2 sm:w-[42%] lg:w-[26%]"
            >
              <Image
                src={img.src}
                alt={img.cap}
      ...
      <section className="bg-ink text-paper">
        <div className="shell py-8 sm:py-14">
          <Reveal className="max-w-2xl">
            ...
            <h2 className="display balance mt-3 text-[clamp(1.45rem,4.6vw,3rem)]">
              Built for how business actually{" "}
              <span className="serif-em text-crimson">works</span>.
            </h2>
          </Reveal>
          <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-[2px] border border-white/15 bg-white/15 lg:grid-cols-4">
            {WHY_NEXIS.map((w, i) => (
              <Reveal key={w.t} delay={i * 70} className="bg-ink p-4 sm:p-7">
                <p className="display text-[1.9rem] text-crimson sm:text-5xl">
                  {w.k}
                </p>
                <h3 className="mt-2 font-serif text-[0.98rem] leading-tight sm:mt-4 sm:text-xl">
                  {w.t}
                </h3>
                <p className="mt-1.5 text-[0.8rem] leading-relaxed text-white/60 sm:mt-3 sm:text-[0.92rem]">
                  {w.d}
                </p>
              </Reveal>
            ))}
        ...
        <div className="grid gap-5 lg:grid-cols-12 lg:gap-14">
          <div className="lg:col-span-4">
            <Reveal>
              ...
              <h2 className="display balance mt-3 text-[clamp(1.45rem,4.4vw,2.8rem)]">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="shell relative py-9 sm:py-16">
          <Reveal className="max-w-2xl">
            ...
            <h2 className="display balance mt-3 text-[clamp(1.65rem,5.2vw,3.6rem)] text-white">
        ...
        <div className="shell py-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <Image
              ...
              width={400}
              height={105}
              className="h-6 w-auto self-start sm:h-7"
            />
            <div className="flex flex-wrap gap-x-5 gap-y-2 text-[0.78rem] text-white/60 sm:text-[0.8rem]">
          ...
          <p className="mt-5 max-w-4xl text-[0.72rem] leading-relaxed text-white/45">
